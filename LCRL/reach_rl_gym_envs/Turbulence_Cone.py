...
class TurbulenceCone(gym.Env):
    ...
    def cone_constraint(self, state_batch):
        cone = np.sqrt((state_batch[:, 0] - state_batch[:, 6])**2 + \
                        (state_batch[:, 2] - state_batch[:, 8])**2) - \
                        (1+max(0,(state_batch[:, 10]-state_batch[:, 4])))*\
                        self.safe_cone_radius
        return cone
    ...
    def step(self, act, evaluate_state = None):
        ...
        next_state[0] = state[0] + dt * state[1]
        next_state[1] = state[1] + dt * control_gain_1*act[0]
        
        next_state[2] = state[2] + dt * state[3]
        next_state[3] = state[3] + dt * control_gain_1*act[1]
        
        next_state[4] = state[4] + dt * state[5]
        next_state[5] = state[5] + dt * control_gain_1*act[2]
        # dynamics of the other drone under disturbance:
        next_state[6] = state[6] + dt * state[7]
        next_state[7] = state[7] + dt * control_gain_2*act_other[0] + dt * disturbance_gain*act[3]
        next_state[8] = state[8] + dt * state[9]
        next_state[9] = state[9] + dt * control_gain_2*act_other[1] + dt * disturbance_gain*act[4] #dt * (-0.9170*self.state[8]-1.6821*self.state[9] )
        next_state[10] = state[10] + dt * state[11]
        next_state[11] = state[11] + dt * control_gain_2*act_other[2] + dt * disturbance_gain*act[5]
