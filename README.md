...
We propose a new reach-avoid value function, which is Lipschitz continuous, and its Bellman equation is a contraction mapping. Our method does not need to anneal the time discount factor to 1, which is commonly used in prior works. Assuming the learning quality is reasonably good, our method only requires 1/N total training time of prior works, where N is their number of time-discount factor annealing times. Furthermore, we also propose two post-learning reach-avoid set verification methods.
