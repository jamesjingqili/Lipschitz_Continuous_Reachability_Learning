...
We propose a new reach-avoid value function, which is Lipschitz continuous, and its Bellman equation is a contraction mapping. Our method does not need to anneal the time discount factor to 1, which is commonly used in prior works. In theory, assuming the learning quality is reasonably good, our training process could require only 1/N of the total training time compared to prior works, where N represents the number of times of annealing the time discount factor.  We also propose two post-learning reach-avoid set certification methods for verifying if a set of states is within the ground truth reach-avoid set.
