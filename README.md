...
We propose a new reach-avoid value function, which is Lipschitz continuous, and its Bellman equation is a contraction mapping. Our method does not need to anneal the time discount factor to 1, which is commonly used in prior works. This simplifies the training procedure and can save training time. We also propose two post-learning reach-avoid set certification methods for verifying if all elements of a learned reach avoid set can safely reach the target set under the worst-case disturbance.
